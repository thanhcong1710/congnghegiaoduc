...
<template>
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    ...
    <component :is="scrollbarTag" class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      <div class="p-6">
        <label for="" class="vs-input--label">Chuyên đề</label>
         <v-select
                id="topic-options"
                v-model="topic_id"
                :dir="$vs.rtl? 'rtl' : 'ltr'"
              ...
              />
        <span class="text-danger text-sm" v-show="errors.has('topic_id')">{{ errors.first('topic_id') }}</span>
      </div>
      <div class="p-6 pt-0">
    ...
    submitData () {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          this.$vs.loading()
          axios.p('/api/tests/create',obj).then((response) => {
