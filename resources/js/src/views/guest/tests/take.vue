        ...
        <vx-card no-shadow class="test-overview">
          <div class="text-center">
              <vs-col vs-justify="center" vs-align="center" vs-lg="6" class="p-2 pt-3" style="border-right: 1px solid #ccc;">
                Số câu đã làm <br>
                <span style="font-size:30px">0/{{quizs.length}}</span>
              </vs-col>
              ...
              <div class="text-left box-select-quiz">
                <div class="quiz-icon" v-for="(item, index) in quizs" :key="index">{{index+1}}</div>
              </div>
              <div class="text-center box-action pr-3 pl-3">
                <vs-button color="warning" type="filled" class="w-full" >NỘP BÀI</vs-button>
              </div>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        axios.g(`/api/test/session-info/${this.$route.params.code}`)
          .then((response) => {
            ...
            if (response.data.status) {
              ...
              this.getDataQuiz(this.test.test_id)
            } else {
              this.$router.push({
                name: 'page-error-404'
              }).catch(() => {})
      ...
      getDataQuiz(test_id) {
        this.$vs.loading()
        axios.g(`/api/test/session-quiz/${test_id}`)
          .then((response) => {
            this.$vs.loading.close()
