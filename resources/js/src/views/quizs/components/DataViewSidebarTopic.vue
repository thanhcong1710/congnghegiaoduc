...
<template>
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    <div class="mt-6 flex items-center justify-between px-6">
        <h4>{{ Object.entries(this.data).length === 0 ? "THÊM MỚI" : "CẬP NHẬT" }} CHUYÊN ĐỀ</h4>
    ...
    <component :is="scrollbarTag" class="scroll-area--data-list-add-new" :settings="settings" :key="$vs.rtl">
      ...
      <div class="p-6">
        <vs-input label="Tên chuyên đề" type="text" v-model="title" class="mt-2 w-full" name="title" v-validate="'required'" />
    ...
    submitData () {
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          this.$vs.loading()
          axios.p('/api/topics/create',obj).then((response) => {
